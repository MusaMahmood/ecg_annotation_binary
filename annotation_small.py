...
description = DATASET + '_annotate_mini'
...
def build_annotator(input_channels=1, output_channels=1):
    ...
    # Downsampling:
    d1 = conv_layer(input_samples, 32 // 4, 8, 2)
    d2 = conv_layer(d1, 64 // 4, 8, 2)
    d3 = conv_layer(d2, 128 // 4, 8, 2)
    d4 = conv_layer(d3, 256 // 4, 8, 2)

    # Now Upsample:
    u1 = deconv_layer(d4, d3, 128 // 4, f_size=8)
    u2 = deconv_layer(u1, d2, 64 // 4, f_size=8)
    u3 = deconv_layer(u2, d1, 32 // 4, f_size=8)
